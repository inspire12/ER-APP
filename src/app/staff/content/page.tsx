...
export default function ContentPage() {
  ...
  return <>
    <h1>질환별 안내 콘텐츠</h1><p className="muted">환자에게 반복해서 제공할 검수 문구를 관리합니다.</p>
    <div className="grid"><aside className="card"><h2>등록된 질환</h2><button className="secondary" onClick={() => setForm(empty)}>새 콘텐츠</button>
      {templates.map((item) => <button key={item.id} className="secondary" style={{ width: "100%", marginTop: 8, justifyContent: "space-between" }} onClick={() => setForm(item)}><span>{item.name}</span><small>{item.active ? "사용" : "중지"}</small></button>)}
    </aside>
    <section className="card"><h2>{form.id ? "콘텐츠 수정" : "새 콘텐츠 등록"}</h2><form onSubmit={save}>
      ...
      <label style={{ display: "flex", gap: 8 }}><input style={{ width: 18 }} type="checkbox" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} />진료 화면에서 사용</label>
